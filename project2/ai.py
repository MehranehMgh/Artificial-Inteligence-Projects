...
class Agent:
    ...
    def act(self):
        ...
        root = self.create_tree(food_map, score, cost, agent_body, 30, prev_action, snake_shekam, None)
        print('tree created')
        ...
        if depth != 0:
            for action in self.actions:
                if self.is_possible(action, agent_body, max_x, max_y, shekam):
                    body, new_score, new_shekam = self.get_news(action, food_map, agent_body, score, prev_action,
                                                                shekam)
                    child = self.create_tree(food_map, new_score, cost + 1, body, depth - 1, action, new_shekam,
                                             root)
    ...
    def get_best_action(self, root: Node):
        queue = [root]
        max_score, best_move = -100, ''
        while len(queue) > 0:
            node = queue.pop(0)
            queue.extend(node.get_children())
            if node.score >= self.win_score:
                best_move = self.get_root_action(node)
                break
    ...
    def is_possible(self, action, agent_body, max_x, max_y, shekam):
        ...
        if shekam != 0:
            ...
            if new_head in agent_body[1:]:
                return False
        if new_head[0] < 0 or new_head[1] < 0 or new_head[0] == max_x or new_head[1] == max_y:
            return False
        return True
    ...
    def get_news(self, action, food_map, agent_body, score, prev_action, shekam):
        new_head = self.get_new_head(action, agent_body)
        ...
        if shekam == 0:
            if len(new_body) == 1:
                new_score = score + self.food_score_multi * food_map[new_head[0]][new_head[1]] + self.food_add_score
    ...
    def get_root_action(self, node):
        if node.parent is None:
            return node.action
        if node.parent.is_root():
            return node.action
        return self.get_root_action(node.parent)
